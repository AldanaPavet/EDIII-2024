 ...
 * TAREA DE ASISTENCIA DE LA CLASE 12/09
 * 
...
void configure_port(void)
{
    ...
    pin_cfg_struct.Pinmode = PINSEL_PINMODE_PULLUP;   /* Pin mode es pull-up */
...
void configure_adc(void)
{
    ...
    /* Configurar el ADC para que se inicie cuando ocurra el evento de match del Timer0 */
    ADC_StartCmd(LPC_ADC, ADC_START_ON_MAT01); /* Start ADC on match event from Timer0 match channel 0 */
}

/**
 * @brief Configure Timer0 to trigger an ADC conversion every 60 seconds.
 */
void configure_timer_and_match(void)
{
    ...
    match_cfg_struct.ExtMatchOutputType = TIM_EXTMATCH_TOGGLE; /* Toggle external match output on match */
